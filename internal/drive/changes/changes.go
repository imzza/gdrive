...
func ListChanges(drv *drive.Drive, args ListChangesArgs) error {
	if args.Now {
		pageToken, err := GetChangesStartPageToken(drv)
		if err != nil {
			return err
		}
	...
	changeList, err := drv.Service.Changes.List(args.PageToken).PageSize(args.MaxChanges).RestrictToMyDrive(true).Fields("newStartPageToken", "nextPageToken", "changes(fileId,removed,time,file(id,name,md5Checksum,mimeType,createdTime,modifiedTime))").Do()
	if err != nil {
...
func GetChangesStartPageToken(drv *drive.Drive) (string, error) {
	res, err := drv.Service.Changes.GetStartPageToken().Do()
	if err != nil {
...
func PrintChanges(args PrintChangesArgs) {
	...
	for _, c := range args.ChangeList.Changes {
		...
		fmt.Fprintf(w, "%s\t%s\t%s\t%s\n",
			c.FileId,
			common.TruncateString(name, args.NameWidth),
			action,
			common.FormatDatetime(c.Time),
		)
	}
...
func nextChangesPageToken(cl *gdrive.ChangeList) (string, bool) {
	if cl.NextPageToken != "" {

...
func DownloadSync(drv *drive.Drive, args DownloadSyncArgs) error {
	...
	err = createMissingLocalDirs(files, args)
	if err != nil {
		return err
	}

	// Download missing files
	err = downloadMissingFiles(drv, files, args)
	if err != nil {
		return err
	}

	// Download files that has changed
	err = downloadChangedFiles(drv, changedFiles, args)
	if err != nil {
		return err
	}
	...
	if args.DeleteExtraneous {
		err = deleteExtraneousLocalFiles(files, args)
		if err != nil {
			return err
		}
	}
...
func getSyncRoot(drv *drive.Drive, rootId string) (*gdrive.File, error) {
	fields := []googleapi.Field{"id", "name", "mimeType", "appProperties"}
	f, err := drv.Service.Files.Get(rootId).Fields(fields...).Do()
	if err != nil {
...
func createMissingLocalDirs(files *syncFiles, args DownloadSyncArgs) error {
...
func downloadMissingFiles(drv *drive.Drive, files *syncFiles, args DownloadSyncArgs) error {
	...
	for i, rf := range missingFiles {
		...
		err = downloadRemoteFile(drv, rf.file.Id, absPath, args, 0)
		if err != nil {
			return err
		}
	}
...
func downloadChangedFiles(drv *drive.Drive, changedFiles []*changedFile, args DownloadSyncArgs) error {
	...
	for i, cf := range changedFiles {
		...
		err = downloadRemoteFile(drv, cf.remote.file.Id, absPath, args, 0)
		if err != nil {
			return err
		}
	}
...
func downloadRemoteFile(drv *drive.Drive, id, fpath string, args DownloadSyncArgs, try int) error {
	if args.DryRun {
		return nil
	}
	...
	res, err := drv.Service.Files.Get(id).Context(ctx).Download()
	if err != nil {
		if common.IsBackendOrRateLimitError(err) && try < common.MaxErrorRetries {
			common.ExponentialBackoffSleep(try)
			try++
			return downloadRemoteFile(drv, id, fpath, args, try)
	...
	progressReader := common.GetProgressReader(res.Body, args.Progress, res.ContentLength)

	// Wrap reader in timeout reader
	reader := timeoutReaderWrapper(progressReader)

	// Ensure any parent directories exists
	if err = common.Mkdir(fpath); err != nil {
		return err
	}
...
func deleteExtraneousLocalFiles(files *syncFiles, args DownloadSyncArgs) error {

...
func prepareSyncFiles(drv *drive.Drive, localPath string, root *gdrive.File, cmp FileComparer) (*syncFiles, error) {
	...
	go func() {
		files, err := prepareRemoteFiles(drv, root, "")
		remoteCh <- struct {
			files []*RemoteFile
			err   error
		}{files, err}
...
func IsSyncFile(drv *drive.Drive, id string) (bool, error) {
	f, err := drv.Service.Files.Get(id).Fields("appProperties").Do()
	if err != nil {
...
func prepareRemoteFiles(drv *drive.Drive, rootDir *gdrive.File, sortOrder string) ([]*RemoteFile, error) {
...
func prepareRemoteRelPaths(root *gdrive.File, files []*gdrive.File) (map[string]string, error) {
	...
	fileLookup := map[graph.NI]*gdrive.File{}

	// All files includes root dir
	allFiles := append([]*gdrive.File{root}, files...)
...
func checkFiles(files []*gdrive.File) error {
...
func (self *syncFiles) filterMissingLocalDirs() []*RemoteFile {
	...
	for _, rf := range self.remote {
		if common.IsDir(rf.file) && !self.existsLocal(rf) {
...
func (self byLocalPathLength) Less(i, j int) bool {
	return common.PathLength(self[i].relPath) < common.PathLength(self[j].relPath)
}
...
func formatConflicts(conflicts []*changedFile, out io.Writer) {
	...
	for _, cf := range conflicts {
		fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%s\n",
			common.TruncateString(cf.local.relPath, 60),
			common.FormatSize(cf.local.Size(), false),
			common.FormatSize(cf.remote.Size(), false),

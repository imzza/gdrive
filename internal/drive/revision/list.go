...
func ListRevisions(drv *drive.Drive, args ListRevisionsArgs) (err error) {
	revList, err := drv.Service.Revisions.List(args.Id).Fields("revisions(id,keepForever,size,modifiedTime,originalFilename)").Do()
	if err != nil {
...
func PrintRevisionList(args PrintRevisionListArgs) {
	...
	for _, rev := range args.Revisions {
		fmt.Fprintf(w, "%s\t%s\t%s\t%s\t%s\n",
			rev.Id,
			common.TruncateString(rev.OriginalFilename, args.NameWidth),
			common.FormatSize(rev.Size, args.SizeInBytes),
			common.FormatDatetime(rev.ModifiedTime),
			common.FormatBool(rev.KeepForever),
		)
	}
